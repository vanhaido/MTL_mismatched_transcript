...
# This program reads VIETNAMESE text and converts it to IPA characters.
...
#Define a file path
w2pdict = os.environ["SBS_DATADIR"] + "/" +  "tkekona/" + "vietnamese/data/dict.txt"
p2idict = os.environ["SBS_DATADIR"] + "/" +  "tkekona/" + "vietnamese/data/g2p_dict.txt"
oovchar = open("conf/vietnamese/oovchar.txt", 'w')
oovpronun = open("conf/vietnamese/oovpronun.txt", 'w')
results = open("conf/vietnamese/cntrans.txt", 'w')
...
def main(argv):
	...
	try:
		...
		print 'sbs_create_phntrans_VN_phone.py -g <grammerToPhone> -u <utterances> -t <transcriptDirectory>'
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'sbs_create_phntrans_VN_phone.py -g <grammerToPhone> -u <utterances> -t <transcriptDirectory>'
