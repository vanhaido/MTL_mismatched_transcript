...
#if [ $# != 4 ]; then
if [ $# != 5 ]; then
   echo "usage: local/align_fmllr.sh <data-dir> <lang-dir> <src-dir> <align-dir>"
   echo "e.g.:  local/align_fmllr.sh data/train data/lang exp/tri1 exp/tri1_ali"
...
if $use_graphs; then
  echo "unexpected, see local/align_fmllr_pt.sh line 99" && exit 1;
:<<'END'
...
END
  ...
  if [ $stage -le 0 ]; then
    ...
    $cmd JOB=1:$nj $dir/log/text_pt.JOB.log \
      ...
      '{key=value=$1; value=value".GTPLM.fst"; print key"\t"dir_fsts"/"value}' \
