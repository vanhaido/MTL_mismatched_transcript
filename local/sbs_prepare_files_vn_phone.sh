...
#exit 1;
	...
	local/sbs_create_phntrans_VN_phone.py --g2p conf/${full_name}/g2pmap.txt --utts $tmpdir/downsample/${x}_basenames_wav --transdir "$TRANSDIR/${full_name}" | LC_ALL=en_US.UTF-8 local/uniphone.py | 		sed 's/sil//g' | sed 's/   */ /g' | sed 's/^ *//g' | sed 's/ *$//g'> $tmpdir/${LCODE}_${x}.trans        
	#sleep 0.1;
	#tonal_phone=0
