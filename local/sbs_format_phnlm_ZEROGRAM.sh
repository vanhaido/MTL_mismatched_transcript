...
lmdir=data/${LCODE}/local/lm_ZEROGRAM
...
echo "Preparing phone LMs..."
  ...
  ! which build-lm.sh 2>/dev/null  && \
	...
	cut -f2- $srcdir/train_text_ZEROGRAM | sed -e 's:^:<s> :' -e 's:$: </s>:' \
	> $srcdir/lm_train.text
	build-lm.sh -i $srcdir/lm_train.text -n 1 -o $tmpdir/lm_phone.gz
...
test=data/${LCODE}/lang_test_ZEROGRAM
mkdir -p $test
