...
#labels_tr="ark:/media/hai/HD/projects/BABEL/exp/GO/exp_train0.25_perturb3_0.9/HUMAN_MAN_12_GEORGIAN_10hr_1transcriber_GOP_threshold0.5/tri3cpt_ali/ali.bin.all"
#labels_cv="ark:/media/hai/HD/projects/BABEL/exp/GO/exp_train0.25_perturb3_0.9/HUMAN_MAN_12_GEORGIAN_10hr_1transcriber_GOP_threshold0.5/tri3cpt_ali/ali.bin.all"
...
if [ ! -z "$feature_transform" ]; then
  ...
  echo "Generated feature transform using splice = $feature_transform"
...
if [ ! -z "$nnet_proto" ]; then 
  ...
  nnet-initialize --binary=${nnet_binary} $nnet_proto $nnet_init 2>$log || { cat $log; exit 1; } 
fi

if [[ -z "$nnet_init" && -z "$nnet_proto" ]]; then
  echo "Supplied no nnet init and no nnet proto. Hence, init nnet using network prototype $nnet_proto";
  ...
  echo "num tgt = $num_tgt, hid layers = $hid_layers, hid dim = $hid_dim, network_type = $network_type"
  ...
  if [ ! -z $dbn ]; then
    ...
    nnet-concat --binary=${nnet_binary} $dbn $nnet_init_old $nnet_init || exit 1 
  fi  
fi
