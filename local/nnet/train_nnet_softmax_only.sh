...
## Van Hai DO - ADSC
#we remove the softmax layer of the source DNN and add a random weight softmax on this
#however, we dont want to update the hidden layer weight, hence we make the hidden layers as a fixed transform, 
# the net now is only the softmax layer
...
usage="Usage: $0 <hmm model> <input_network> <output_network> <oldtransform> <newtransform> > "
##e.g. renew_nnet_softmax.sh exp/tri3b/final.mdl nnet.old nnet.new

[[ $# -eq 5 ]] || { echo $usage; exit 1; }
...
echo "Concatenating two networks => make the new transform"
#Connect the two networks
#Make sure that the oldnn_outdim = softmax_indim dimensions 
nnet-concat $oldtransform ${nnet_stripped} ${newtransform}
echo "New transform is stored at" ${newtransform}
echo "Done"
echo
#============================================
...
nnet-initialize --binary=false $nndir/${tempnn}.proto $newnn
echo "Done"
echo
