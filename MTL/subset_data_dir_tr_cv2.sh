...
seed=778 # use seed for speaker shuffling
# End configuration.
...
if [ $# != 3 ]; then
  echo "Usage: $0 [--cv-spk-percent P|--cv-utt-percent P|--cv-utt-percent-random P] <srcdir> <traindir> <crossvaldir>"
  ...
  echo "  --cv-utt-percent_random P  RANDOM Cross Validation portion of the total utterances, default is 10% (i.e. P=10)"
  echo "  "
  exit 1;
fi
...
if $uttbase; then
  echo "divide the training set to training and cv set 90-10 based on utterances" 
  ...
  #move the boundary so it is located on speaker change
	#vanhai  in case of very few speakers just divide by utterances
...
if $uttbase_random; then
	echo "=============================================================================="
  echo "divide the training set to training and cv set 90-10 based on utterances RANDOMLY" 
	echo "=============================================================================="
  if [ ! -f $srcdir/utt2spk ]; then
    echo "$0: no such file $srcdir/utt2spk"
    exit 1;
...
#shuffle the utt list
awk '{print $1}' $srcdir/utt2spk | shuffle_list.pl --srand $seed > $trndir/_tmpf_randutt
#split the train/cv
head -n $P_utt $trndir/_tmpf_randutt > $cvdir/_tmpf_cvutt
tail -n $N_head $trndir/_tmpf_randutt > $trndir/_tmpf_trainutt
...
#clean-up
rm -f $trndir/_tmpf_randutt $trndir/_tmpf_trainutt $cvdir/_tmpf_cvutt
...
#===========================================
echo "divide the training set to training and cv set 90-10 based on speakers"
